...
void FFR::Reinit(float centerShiftX, float centerShiftY){
    // EyeNexus-Client-Side FSD: Reinitialize Foveated Spatial Decompression (FSD)
    // Recalculate foveation variables based on the received center shift
    ...
    // Rebuild the shader string with new foveation parameters
    auto ffrCommonShaderStr = string_format(FFR_COMMON_SHADER_FORMAT,
                                            fv.targetEyeWidth,
                                            fv.targetEyeHeight,
                                            fv.optimizedEyeWidth,
    ...
    // EyeNexus-Client-Side FSD: Use the Decompress Axis Aligned Fragment Shader to reverse the FSC effect
    auto decompressAxisAlignedShaderStr =
